...
class DFHandler:
	...
	def generate_headers(self):
		...
		df2_header = self.ID_COL + [str(i) for i in
									range(self.df1.columns.size, self.df1.columns.size + self.df2.columns.size - 1)]
...
class KmeansPPInitializer:
	...
	def add_new_random_centroid(self):
		...
		self.clusters_df = pd.concat(objs=[self.clusters_df, self.datapoints_df.iloc[new_centroid_index]],
									 ignore_index=True, axis=0)

	# Calculates and sets the new D values of all non-cluster vectors
	def calc_d(self):
		distances = self.euclidian_distance(point1=self.datapoints_df[self.datapoints_df[IS_CENTROID_COL_NAME == 0]],
											point2=self.clusters_df.tail(1))
	...
	def set_initialized_centroids_idx_arr(self):
		self.initialized_centroids_idx_arr = self.datapoints_df.index[
			self.datapoints_df[IS_CENTROID_COL_NAME] == 1].tolist()
...
class KmeansPPRunner:
	...
	def runKmeansPP(self):
		...
		self.final_clusters = mykmeanssp.run_kmeans(self.initialized_Kmeans.datapoints_df,
													self.initialized_Kmeans.clusters_df, self.initialized_Kmeans.iter,
													self.initialized_Kmeans.k, self.initialized_Kmeans.n,
